...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./org3-artifacts/crypto-config/ channel-artifacts/org3.json connection-*
...
function generateCerts() {
  ...
  set -x
  if [[ "${GMSM}" == "true" ]]; then
    cryptogen generate --config=./crypto-config.yaml --crypto=SW/SM2/SM3-256
  else
...
function generateChannelArtifacts() {
  ...
  set -x
  if [[ "${GMSM}" == "true" ]]; then
    configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME -crypto SW/SM2/SM3-256
  else
  ...
  set -x
  if [[ "${GMSM}" == "true" ]]; then
    configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP -crypto SW/SM2/SM3-256
  else
  ...
  if [[ "${GMSM}" == "true" ]]; then
    configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
        ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP -crypto SW/SM2/SM3-256
  else
    configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
