...
addOSN() {
    ...
    cat config.json |  jq '.channel_group.groups.Orderer.values.ConsensusType.value.metadata.consenters += [{"identity": "'$ecert'", "consenter_id": 5, "client_tls_cert": "'$tlscert'", "host": "orderer5.example.com", "port": 7050, "server_tls_cert": "'$tlscert'"}] ' > modified_config.json
}
...
channelConfig() {
    ...
    configtxlator compute_update --channel_id "mychannel" --original config.pb --updated modified_config.pb --output orderer5AdditionOrRemoval.pb
    configtxlator proto_decode --input orderer5AdditionOrRemoval.pb --type common.ConfigUpdate | jq . > orderer5AdditionOrRemoval.json
    echo '{"payload":{"header":{"channel_header":{"channel_id":"mychannel", "type":2}},"data":{"config_update":'$(cat orderer5AdditionOrRemoval.json)'}}}' | jq . > orderer5AdditionOrRemovalInEnvelope.json
    ...
    peer channel update -f orderer5AdditionOrRemovalInEnvelope.pb -c "mychannel" -o orderer.example.com:7050 --tls --cafile $ORDERER_CA
}